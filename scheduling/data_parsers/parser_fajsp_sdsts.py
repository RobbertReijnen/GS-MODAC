...
from scheduling.scheduling_environment.machine import Machine
from scheduling.scheduling_environment.job import Job
...
def parse(JobShop, instance, from_absolute_path=False):
    ...
    #load sdsts:
    base_instance = os.path.basename(instance)
    sdsts_path = base_path.joinpath('data/fajsp_sdsts/sdsts/' + base_instance)

    # Read CSV file into a nested list
    sdsts = []
    with open(sdsts_path, 'r', encoding='utf-8') as file:
        csv_reader = csv.reader(file)
        for row in csv_reader:
            # Convert strings representing lists into actual lists
            converted_row = [ast.literal_eval(item) if item.startswith('[') and item.endswith(']') else int(item) for
                             item in row]
            sdsts.append(converted_row)

    JobShop.add_sequence_dependent_setup_times(sdsts)
